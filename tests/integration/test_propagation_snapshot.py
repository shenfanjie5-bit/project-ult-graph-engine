...
def test_compute_graph_snapshots_runs_fundamental_pagerank_on_promoted_graph() -> None:
    ...
    with Neo4jClient(load_config_from_env()) as client:
        ...
        try:
            ...
            status_snapshot = build_graph_snapshot("cycle-1", 1, client)
            graph_status = Neo4jGraphStatus(
                graph_status="ready",
                graph_generation_id=status_snapshot.graph_generation_id,
                node_count=status_snapshot.node_count,
                edge_count=status_snapshot.edge_count,
                key_label_counts=status_snapshot.key_label_counts,
                checksum=status_snapshot.checksum,
